...
bool Pipeline::hasDependency(void)
{
    ...
    for (int i = EXEC; i < WB; i++)
    {
        ...
        else
        {
            ...
            if((i==MEM||i==EXEC)&&(pipeline[i].inst->dest == pipeline[DECODE].inst->src1 ||
                        pipeline[i].inst->dest == pipeline[DECODE].inst->src2))
            {
...
/*            if(i==EXEC&&(pipeline[i].inst->dest == pipeline[DECODE].inst->src1 ||
                        pipeline[i].inst->dest == pipeline[DECODE].inst->src2))
            {
                return false;
            }*/
...
/*              std::cout<<"true"<<std::endl;
                std::cout<<i<<std::endl;
                std::cout<<pipeline[i].inst->type<<std::endl;
                std::cout<<pipeline[DECODE].inst->type<<std::endl;*/
                return true;
            }
        }
